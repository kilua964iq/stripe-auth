...
def txt_progress(
    ...
    return (
        ...
        f"├──────────────────────────┤\n"
        f"│ 📊 Progress: [{bar}] {pct}%\n"
        f"│ ✅ Approved: <b>{approved}</b>  │  ❌ Declined: <b>{declined}</b>\n"
        f"│ 💳 <code>{current}</code>\n"
        f"│ 📝 Response: <b>{response}</b>\n"
        f"│ ⏱ Time: {elapsed:.2f}s\n"
        f"├──────────────────────────┤\n"
...
def txt_approved_card(cc: str, result: str, elapsed: float) -> str:
    bin6 = cc.split("|")[0][:6]
    return (
        f"<tg-emoji emoji-id='{ICO_APPROVED}'>✅</tg-emoji> <b>#Stripe_Auth</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💳 𝐂𝐚𝐫𝐝: <code>{cc}</code>\n"
        f"📝 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: <b>{result}</b>\n"
        f"⏱ 𝐓𝐢𝐦𝐞: {elapsed:.2f}s\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"{fmt_bin(bin6)}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
    ...
    for i, cc in enumerate(cards, 1):
        ...
        if is_approved(result):
            ...
            try:
                bot.send_message(
                    chat_id,
                    txt_approved_card(cc, result, elapsed),
                    parse_mode="HTML",
                )
            except Exception:
                pass
        ...
        try:
            bot.edit_message_text(
                txt_progress(i, total, ap_count, dc_count,
...
def cmd_chk(message):
    ...
    def _do():
        ...
        if is_approved(result):
            icon = f"<tg-emoji emoji-id='{ICO_APPROVED}'>✅</tg-emoji>"
            color_text = "<b>✅ APPROVED</b>"
        else:
            icon = f"<tg-emoji emoji-id='{ICO_DECLINED}'>❌</tg-emoji>"
            color_text = "<b>❌ DECLINED</b>"

        text = (
            f"{icon} <b>#Stripe_Auth</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"💳 𝐂𝐚𝐫𝐝: <code>{cc}</code>\n"
            f"📝 𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: {color_text}\n"
            f"💬 𝐌𝐬𝐠: <code>{result}</code>\n"
            f"⏱ 𝐓𝐢𝐦𝐞: {elapsed:.2f}s\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"{fmt_bin(bin6)}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
        ...
        try:
            bot.edit_message_text(
                text, prog.chat.id, prog.message_id,
                parse_mode="HTML",
                reply_markup=kb_home(),
            )
        except Exception:
